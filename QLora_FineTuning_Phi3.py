...
# # for i, layer in enumerate(clipvisionmodel.vision_model.layers):
# #     print(f"Layer {i}: {layer}")
# for name, module in clipvisionmodel.named_modules():
#     print(name, module)

# print("clipvisionmodel -2 layer:", clipvisionmodel.named_modules()[-2])
# print("clipvisionmodel -1 layer:", clipvisionmodel.named_modules()[-1])
...
class ImageTextDatasetForCausalLM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.clip_processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
        # Load and preprocess the image
        # image = self.clip_processor(images=Image.open(image_files[0]), return_tensors="pt")
        # print("image keys:", image.keys())

        # # Prepare the image input for the CLIP model
        # image_input = {
        #     "pixel_values": image["pixel_values"].squeeze(0)
        # }


        image_forward_out = clipvisionmodel(image.unsqueeze(0), output_hidden_states=True, output_attentions=True)
        # print("clipvisionmodel:", clipvisionmodel)
        image_features = image_forward_out.hidden_states[-1].squeeze(0)
        # print("image_features shape:", image_features.shape)
        image_attention_scores = image_forward_out.attentions
        # print("attention_scores len:", len(image_attention_scores))
        # print("attention_scores:", image_attention_scores)
...
def collate_fn(batch):
    ...
    start_tokens = tokenizer(start_texts, padding=True, truncation=True, max_length=512, return_tensors="pt")
    end_tokens = tokenizer(end_texts, padding=True, truncation=True, max_length=512, return_tensors="pt")
...
# Usage example:
dataset = ImageTextDatasetForCausalLM(clipimageprocessor, "./conversations.csv", "./data/train2014", tokenizer)
...
class MultimodalPhiModel(PreTrainedModel):
    ...
    def from_pretrained(self, pretrained_model_name_or_path, *model_args, debug=False, **kwargs):
        ...
        model = self(base_phi_model, projector, debug=debug)
        return model
...
trainer = MultimodalTrainer(
    model=model,
    args=TrainingArguments(output_dir="./FullOnProj_results",
                           do_train=True,
                           do_eval=True,
                           num_train_epochs=1, 
...
# Start training
trainer.train()
model.save_pretrained("./FullOnProjmodel")
